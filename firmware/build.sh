...
cp "$APP_BIN" "$RELEASE_DIR/openrs-fw-usb_v110.bin"
log "App binary: $(basename "$APP_BIN") → openrs-fw-usb_v110.bin"
...
echo "  Address     File"
echo "  ─────────   ────────────────────────────────────────────"
for f in bootloader.bin partition-table.bin ota_data_initial.bin openrs-fw-usb_v110.bin storage.bin; do
    fp="$RELEASE_DIR/$f"
    [ -f "$fp" ] || continue
    ...
    case "$f" in
        ...
        openrs-fw-usb_v110.bin) ADDR="0x10000"   ;;
