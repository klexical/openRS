...
- **`MeatPiConnection.readSlcanLine` could hang indefinitely** — `socket.soTimeout` only fires when `read()` blocks with no data; if the adapter sent continuous bytes without a `\r` terminator the loop ran forever, freezing the MeatPi connection. Added a 32-byte max-line guard. Fixes [#127](https://github.com/klexical/openRS_/issues/127).
- **`IsoTpBuffer` CF sequence numbers not validated** — the BCM live-data path (0x280B TPMS multi-frame reassembly) accepted Consecutive Frames without checking the sequence nibble. A duplicate or out-of-order CF silently wrote payload at the wrong buffer offset, corrupting TPMS readings. Sequence tracking added matching the existing DTC scanner logic. Fixes [#128](https://github.com/klexical/openRS_/issues/128).
- **ISO-TP First Frame `firstBytes` not capped by `totalLen`** — both connection classes computed bytes to copy from the First Frame as `minOf(6, data.size - 2)`, ignoring `totalLen`. Changed to `minOf(6, totalLen, data.size - 2)`. Fixes [#129](https://github.com/klexical/openRS_/issues/129).
...
- **Misleading `Red` color alias removed** — `val Red get() = Orange` in `Theme.kt` implied a red color but returned `#FF4D00` (orange). Alias removed; all 35 call sites across 11 files now reference `Orange` directly. Addresses sub-item in [#82](https://github.com/klexical/openRS_/issues/82).
- **TempsPage RTR dot animation ticked as no-op when race-ready** — `animateFloat` oscillated between `1f` and `1f` when `isReady` was true, running every frame with zero visual effect. Animation now only runs during warmup; race-ready state uses a static `1f` alpha. Addresses sub-item in [#82](https://github.com/klexical/openRS_/issues/82).
- **MainActivity connection dot animation ran when disconnected** — `rememberInfiniteTransition` ticked continuously regardless of connection state, wasting frames. Animation now only runs when `isConnected`; disconnected/idle states use static alpha. Addresses sub-item in [#82](https://github.com/klexical/openRS_/issues/82).
- **Odometer unit toggle reset on tab switch** — `remember { mutableStateOf(false) }` defaulted to km on every recomposition. Initial value now derives from `prefs.speedUnit == "MPH"` so the toggle matches the user's unit preference. Addresses sub-item in [#82](https://github.com/klexical/openRS_/issues/82).
- **UserPrefs.update race with SharedPreferences** — `AppSettings.saveAll()` ran inside the `StateFlow.update` CAS lambda, so a retry on contention would persist stale data before being discarded. `saveAll` now runs after CAS succeeds, only persisting the winning value. Addresses sub-item in [#83](https://github.com/klexical/openRS_/issues/83).
- **Redundant `themeAccentColor()` wrapper removed** — `MorePage.kt` had a private one-liner delegating to `rsPaintAccent()`. Call sites now invoke `rsPaintAccent()` directly. Addresses sub-item in [#81](https://github.com/klexical/openRS_/issues/81).
- **Conversion magic numbers extracted to named constants** — `0.621371` (km→mi), `101.325` (std atm kPa), `0.14503773` (kPa→PSI), `0.0689476` (PSI→BAR) consolidated into `UnitConversions` object in `UserPrefs.kt`. All 10 occurrences across `UserPrefs.kt`, `DashPage.kt`, `ChassisPage.kt`, and `TripPage.kt` now reference named constants. Addresses sub-item in [#82](https://github.com/klexical/openRS_/issues/82).
